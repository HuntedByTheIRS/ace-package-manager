...
// compress compresses data using bzip2 and returns the compressed bytes.
...
// decompress decompresses bzip2-compressed data and returns the uncompressed bytes.
...
// new_bzip2_reader creates an ArchiveReader configured for reading bzip2-compressed archives from a file.
pub fn new_bzip2_reader(path string) !&archive.ArchiveReader {
	reader := archive.new_reader()
	reader.support_filter_all()!
	reader.support_format_all()!
...
// new_bzip2_writer creates an ArchiveWriter configured for writing bzip2-compressed archives to a file.
pub fn new_bzip2_writer(path string) !&archive.ArchiveWriter {
	w := archive.new_writer()
	w.add_filter_bzip2()!
	w.set_format_pax_restricted()!
	w.open_file(path)!
