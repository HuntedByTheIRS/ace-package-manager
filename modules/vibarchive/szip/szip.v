...
// compress compresses data using gzip (Snappy-framed) compression and returns the compressed bytes.
// libarchive does not expose a dedicated Snappy filter; the gzip filter provides the framing contract
// accepted by the szip format reader.
...
// decompress decompresses gzip (Snappy-framed) data and returns the uncompressed bytes.
...
// new_szip_reader creates an ArchiveReader configured for reading szip-compressed archives from a file.
pub fn new_szip_reader(path string) !&archive.ArchiveReader {
	reader := archive.new_reader()
	reader.support_filter_all()!
	reader.support_format_all()!
...
// new_szip_writer creates an ArchiveWriter configured for writing szip-compressed archives to a file.
pub fn new_szip_writer(path string) !&archive.ArchiveWriter {
	w := archive.new_writer()
	w.add_filter_gzip()!
	w.set_format_pax_restricted()!
	w.open_file(path)!
